...
import com.jbh.lib.R;
import com.jbh.lib.adapter.decoration.GridItemDecoration;
import com.jbh.lib.model.City;
...
public class CityListAdapter extends RecyclerView.Adapter<CityListAdapter.BaseViewHolder> {
    ...
    private static final int VIEW_TYPE_LATELY = 12;

    private Context mContext;
    private List<City> mData;
    private List<HotCity> mHotData;
    private List<LatelyCity> mLatelyData;
    ...
    public CityListAdapter(Context context, List<City> data, List<HotCity> hotData, List<LatelyCity> latelyData, int state) {
        this.mData = data;
        this.mContext = context;
        this.mHotData = hotData;
        this.locateState = state;
        this.mLatelyData = latelyData;
    }
    ...
    public BaseViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        switch (viewType) {
            ...
            case VIEW_TYPE_LATELY:
                view = LayoutInflater.from(mContext).inflate(R.layout.cp_list_item_hot_layout, parent, false);
                return new LatelyViewHolder(view);
            default:
    ...
    public void onBindViewHolder(BaseViewHolder holder, int position) {
        ...
        //最近搜索的
        if (holder instanceof LatelyViewHolder) {
            final int pos = holder.getAdapterPosition();
            final City data = mData.get(pos);
            if (data == null) return;
            GridListAdapter2 mAdapter = new GridListAdapter2(mContext, mLatelyData);
            mAdapter.setInnerListener(mInnerListener);
            ((LatelyViewHolder) holder).mRecyclerView.setAdapter(mAdapter);
        }
    }
    ...
    public int getItemViewType(int position) {
        if (TextUtils.equals("定", mData.get(position).getSection().substring(0, 1)))
            return VIEW_TYPE_CURRENT;
        if ( TextUtils.equals("最", mData.get(position).getSection().substring(0, 1)))
            return VIEW_TYPE_LATELY;
    ...
    public static class LatelyViewHolder extends BaseViewHolder {
        RecyclerView mRecyclerView;

        LatelyViewHolder(View itemView) {
            super(itemView);
            ...
            mRecyclerView.setLayoutManager(new GridLayoutManager(itemView.getContext(),
                    GridListAdapter2.SPAN_COUNT, LinearLayoutManager.VERTICAL, false));
            int space = itemView.getContext().getResources().getDimensionPixelSize(R.dimen.cp_grid_item_space);
            mRecyclerView.addItemDecoration(new GridItemDecoration(GridListAdapter2.SPAN_COUNT,
                    space));
        }
    }
